...
const Services = () => {
  return (
    <div
      className={`${styles.container} ${styles.space} lg:border-b-2 border-b-[#FA6A19] `}
    >
      ...
      <div className="flex flex-col md:flex-row mt-16 md:mt-24 ">
        ...
        <div className="md:w-[50%] flex flex-col items-center justify-evenly gap-3 mt-5">
          {Service.map((s, key) => (
            <div key={key}>
              <h2 className="text-white text-xl md:text-3xl font-bold uppercase text-center">
                {s.title}
              </h2>
              <p className="text-gray-400 text-xs  md:text-lg text-center">
                {s.description}
              </p>
            </div>
          ))}
